...
function _parse(e, t, n, r, i) {
    ...
    function a(t) {
        if (t > h) {
            var n = e.substring(h, t).replace(/&#?\w+;/g, o);
            d && s(h), r.characters(n, 0, t - h), h = t;
        }
    }
    function s(t, n) {
        ...
        try {
            ...
            if (m < 0) {
                if (!e.substr(h).match(/^\s*$/)) {
                    var N = r.doc, E = N.createTextNode(e.substr(h));
                    N.appendChild(E), r.currentElement = E;
                }
                return;
            }
            switch (m > h && a(m), e.charAt(m + 1)) {
              case "/":
                var g = e.indexOf(">", m + 3), T = e.substring(m + 2, g), _ = p.pop();
                g < 0 ? (T = e.substring(m + 2).replace(/[\s<].*/, ""), i.error("end tag name: " + T + " is not complete:" + _.tagName), 
                g = m + 1 + T.length) : T.match(/\s</) && (T = T.replace(/[\s<].*/, ""), i.error("end tag name: " + T + " maybe not complete"), 
                g = m + 1 + T.length);
              ...
              case "?":
                d && s(m), g = parseInstruction(e, m, r);
                break;

              case "!":
                d && s(m), g = parseDCC(e, m, r, i);
                break;

              default:
                d && s(m);
                var S = new ElementAttributes(), O = p[p.length - 1].currentNSMap, R = (g = parseElementStartPart(e, m, S, O, o, i), 
                S.length);
                if (!S.closed && fixSelfClosed(e, g, S.tagName, f) && (S.closed = !0, n.nbsp || i.warning("unclosed xml attribute")), 
                d && R) {
                    for (var A = copyLocator(d, {}), w = 0; w < R; w++) {
                        var C = S[w];
                        s(C.offset), C.locator = copyLocator(d, {});
                    }
                    r.locator = A, appendElementFromSax(S, r, O) && p.push(S), r.locator = d;
                } else appendElementFromSax(S, r, O) && p.push(S);
                "http://www.w3.org/1999/xhtml" !== S.uri || S.closed ? g++ : g = parseHtmlSpecialContent(e, g, S.tagName, o, r);
            }
        } catch (e) {
            i.error("element parse error: " + e), g = -1;
        }
        g > h ? h = g : a(Math.max(m, h) + 1);
    }
}
...
function serializeToString(e, t, n, r, i) {
    ...
    switch (e.nodeType) {
      ...
      case DOCUMENT_TYPE_NODE:
        var h = e.publicId, m = e.systemId;
        if (t.push("<!DOCTYPE ", e.name), h) t.push(' PUBLIC "', h), m && "." != m && t.push('" "', m), 
